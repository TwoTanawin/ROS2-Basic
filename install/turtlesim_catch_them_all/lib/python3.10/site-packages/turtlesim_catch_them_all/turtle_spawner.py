...
from my_robot_interfaces.srv import CatchTutle
...
class TurtleSpawnerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("spawn_frequncy", 1.0)
        self.declare_parameter("turtle_name_prefix", "turtle")

        self.turtle_name_prefix_ = self.get_parameter("turtle_name_prefix").value
        self.spanw_frequency_ = self.get_parameter("spawn_frequncy").value
        ...
        self.spawn_turtle_timer_ = self.create_timer(
            1.0/self.spanw_frequency_, self.spawn_new_turtle)

        self.catch_turtle_service_ = self.create_service(
            CatchTutle, "catch_turtle", self.callback_catch_turtle)
    ...
    def spawn_new_turtle(self):
        ...
        name = self.turtle_name_prefix_ + str(self.turtle_counter)
    ...
    def call_spawn_server(self, turtle_name, x, y, theta):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for a server ...")
    ...
    def callback_call_kill(self, future, turtle_name):
        try:
            future.result()
            for(i, turtle_name) in enumerate(self.alive_turtles_):
